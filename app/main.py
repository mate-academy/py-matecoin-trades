...
def calculate_profit(trades_file):
    ...
    # two variables for sums are created for better readability
    bought_sum = sum(
        [val["bought"] for val in users if val["bought"]]
    )
    sold_sum = sum(
        [val["sold"] for val in users if val["sold"]]
    )
    matecoin_total = bought_sum - sold_sum

    bought_money_sum = sum(
        [val["bought"] * val["matecoin_price"]
         for val in users if val["bought"]]
    )
    sold_money_sum = sum(
        [val["sold"] * val["matecoin_price"]
         for val in users if val["sold"]]
    )
