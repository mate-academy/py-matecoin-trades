...
def calculate_profit(file_name: str) -> None:
    ...
    for trade in trades_data:
        if trade["bought"] is not None:
            earned_money = earned_money + (Decimal(trade["bought"]) * Decimal(trade["matecoin_price"]))
            matecoin_profit = matecoin_profit + Decimal(trade["bought"])
        elif trade["sold"] is not None:
            earned_money = earned_money - (Decimal(trade["sold"]) * Decimal(trade["matecoin_price"]))
            matecoin_profit = matecoin_profit - Decimal(trade["sold"])

    profit = {
        "earned_money": str(earned_money),
        "matecoin_profit": str(matecoin_profit)
    }
