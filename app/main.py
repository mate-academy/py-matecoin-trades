...
def calculate_profit(file_name: str) -> None:
    ...
    for operation in dict_operation:
        if operation["bought"] is not None:
            dict_profit["earned_money"] -= Decimal(operation["bought"]) * Decimal(
                operation["matecoin_price"]
            )
            dict_profit["matecoin_account"] += Decimal(operation["bought"])
        if operation["sold"] is not None:
            dict_profit["earned_money"] += Decimal(operation["sold"]) * Decimal(
                operation["matecoin_price"]
            )
            dict_profit["matecoin_account"] -= Decimal(operation["sold"])
