...
def calculate_profit(file_name: str) -> None:
    ...
    for symbol in trades_from_file:
        if symbol["bought"] is not None:
            res_earned_money -= decimal.Decimal(symbol["bought"]) *\
                decimal.Decimal(symbol["matecoin_price"])
            res_matecoin_account += decimal.Decimal(symbol["bought"])
        if symbol["sold"] is not None:
            res_earned_money += decimal.Decimal(symbol["sold"]) *\
                decimal.Decimal(symbol["matecoin_price"])
    ...
    with open("profit.json", "w") as new_file:
        json.dump(dict_of_results, new_file, indent=2) #tessss
