...
def calculate_profit(
    trades_file: str = "trades.json", profit_file: str = "profit.json"
) -> None:
    ...
    for data in work_data:
        if data["bought"] is not None:
            sum_bought += decimal.Decimal(data["bought"]) * decimal.Decimal(
                data["matecoin_price"]
            )
            sum_matecoit += decimal.Decimal(data["bought"])
        if data["sold"] is not None:
            sum_sold += decimal.Decimal(data["sold"]) * decimal.Decimal(
                data["matecoin_price"]
            )
