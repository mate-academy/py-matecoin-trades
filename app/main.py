...
def calculate_profit(filename: str):
    ...
    for elem in data:
        if elem["bought"]:
            ...
            money_buy += Decimal(elem["bought"]) * Decimal(elem["matecoin_price"])
        if elem["sold"]:
            total_sold += Decimal(elem["sold"])
            money_sold += Decimal(elem["sold"]) * Decimal(elem["matecoin_price"])
    matecoin_account = total_buy - total_sold
    earned_money = money_sold - money_buy
    output_data = {"earned_money": str(earned_money), "matecoin_account": str(matecoin_account)}
